    ...
    if (!options || !options.settings || options.settings.length === 0) {
        ...
        var instances = Object.keys(options.settings).reduce(function (prev, curr) {
            ...
            prev[curr] = {
                ...
                drop: function(command, callback) {
                    client.drop(_.merge({}, opt.database, command), callback);
                },
                ...
                createIndex: function(command, callback) {
                    client.createIndex(_.merge({}, opt.database, command), callback);
                },
                dropIndex: function(command, callback) {
                    client.dropIndex(_.merge({}, opt.database, command), callback);
                }
            };
            return prev;
        }, {});
